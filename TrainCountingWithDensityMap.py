...
from Models import CrowdCountingWithDensity
...
def train(train_loader, model, optimizer, epoch, cur_lr, cfg, writer_dict, logger, device):
    ...
    # densityMap_losses = AverageMeter()
    # losses = AverageMeter()
    end = time.time()
    ...
    for iter, data in enumerate(train_loader):
        ...
        frames, count_gts, _, density_maps  = data
        frames = frames.squeeze(dim=0).to(device)
        count_gts = count_gts.squeeze(dim=0).float().to(device)
        density_map_gts = density_maps.squeeze(dim=0).float().to(device)
        count_outs, densityMap_outs = model(frames)
        count_loss, densityMap_loss = model.loss(count_outs, count_gts, densityMap_outs, density_map_gts)
        
        count_loss = torch.mean(count_loss)
        densityMap_loss = torch.mean(densityMap_loss)
        loss = count_loss + densityMap_loss
        ...
        if ((iter + 1) % cfg["LOG_PRINT_FREQ"] == 0):
            logger.info(
                'TRAIN - Epoch: [{0}][{1}/{2}] lr: {lr:.7f}\t Batch Time: {batch_time.avg:.3f}s \t Data Time:{data_time.avg:.3f}s \t \
                    Counting Loss:{counting_loss:.5f} \t Density Map Loss:{density_loss:.5f} \t Total Loss:{total_loss:.3f}'.format(epoch, iter + 1, 
                    len(train_loader), lr=cur_lr, batch_time=batch_time, data_time=data_time, counting_loss=count_loss.item(),
                    density_loss=densityMap_loss.item(), total_loss=loss.item()))
        ...
        writer.add_scalars('Train_Losses', {'counting_loss': count_loss.item(), 'density_map_loss': densityMap_loss.item(),
                            'total_loss': loss.item(), 'learning_rate': cur_lr}, global_steps)
...
def main():
    ...
    logger, time_str = create_logger(cfg, 'countingWithDensity_train_')
    logger.info(pprint.pformat(cfg))

    tensorboard_writer_path = os.path.join(cfg["TENSORBOARD_DIR"], "countingWithDensity_train_" + time_str)
    writer_dict = {
    ...
    model = CrowdCountingWithDensity(pretrainedBackbone=cfg["LOAD_ONLY_PRETRAINED_BACKBONE"]).cuda()
    ...
    for epoch in range(cfg["END_EPOCH"]):
        ...
        # save model
        save_model(model, epoch, optimizer, "CrowdCountingWithDensity", cfg["CHECKPOINT_DIR"], isbest=False)
