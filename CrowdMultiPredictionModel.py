...
class CrowdMultiPrediction(nn.Module):
    def __init__(self, pretrainedBackbone=False):
        ...
        self.countingHead = CountingHead(channels=self.backbone_out_channels, layerCount=3)
        self.anomalyClsHead = AnomalyClassificationHead(channels=self.backbone_out_channels, layerCount=3)
        self.countingHeadLoss = nn.MSELoss()
        self.anomalyClsHeadLoss = nn.BCELoss()
        
    def forward(self, frames):
        h1_t, c1_t, h2_t, c2_t = self.lstm_encoder.init_hidden(batch_size=1, image_size=(self.backbone_out_tensor_height, 
    ...
    def loss(self, count_outs, anomaly_out, count_gts, anomaly_gt):
        count_losses = []
        for i in range(len(count_outs)):
            count_losses.append(self.countingHeadLoss(count_outs[i], count_gts[i]))
